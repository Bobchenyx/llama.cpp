        ...
        if (std::smatch match; std::regex_search(ts.tensor, match, pattern)) {
            ...
            if (prev != tstats.end()) {
                ...
                const float prev_mag = std::sqrt(std::inner_product(prev->stats.values.begin(), prev->stats.values.end(),
                    prev->stats.values.begin(), 0.0f));
                const float denom = curr_mag * prev_mag;
                const float cs = (denom > 0.0f) ? dp / denom : 0.0f;
                ts.cossim = cs;
            }
        } else {
            ts.cossim = 0;
...
bool IMatrixCollector::load_imatrix(const char * file_name) {
    ...
    // Read m_last_chunk directly from GGUF metadata if available (avoids division by zero
    // when m_params has not been set, e.g. in --show-statistics mode)
    const int64_t chunk_count_key = gguf_find_key(ctx_gguf, LLM_KV_IMATRIX_CHUNK_COUNT);
    if (chunk_count_key != -1) {
        m_last_chunk = gguf_get_val_u32(ctx_gguf, chunk_count_key);
    } else {
        // Fall back to computing from max activation count
        int64_t max_count = 0;
        for (const auto & stats : m_stats) {
            for (int64_t count : stats.second.counts) {
                if (count > max_count) {
